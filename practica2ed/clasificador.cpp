...
Clasificador::Clasificador(Diccionario& d){
	if(inicio!=NULL)	delete inicio;		//si esta apuntando a algo habria que borrar todos los nodos no solo el primero! 
	inicio=new nodoNum;
	nodoNum *aux;
	if(inicio!=NULL){
		int asig=1;
				...
				if(contador<12&&contador>0){	//nos aseguramos que tenga vocales 
					...
					if(nodoencontrado){			//cuando lo encontremos tenemos que ver donde lo ponemos 
						if(aux->pnp==NULL){
							...
							ayuda=aux->pnp;
							ayuda1=new nodoPal;				//hay que insertarlas en orden alfabetico
							if(ayuda1!=NULL){				//idea:compararla con la de los nodos existentes y meterla por ahi
								strcpy(ayuda1->pal,algo);
								while(strcmp(ayuda->pal,ayuda->pal)<0){
									ayuda=ayuda->sig;
								}
								ayuda2=ayuda->sig;
								if(ayuda2==NULL){
